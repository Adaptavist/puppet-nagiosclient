...
&nagios_return("CRITICAL", "Replication problem: Slave IO not running!") if ($result->{'Slave_IO_Running'} ne "Yes");
&nagios_return("CRITICAL", "Replication problem: Slave SQL not running!") if ($result->{'Slave_SQL_Running'} ne "Yes");
&nagios_return("CRITICAL", "Replication problem: Seconds_Behind_Master=".$result->{'Seconds_Behind_Master'}) if ($result->{'Seconds_Behind_Master'} > 120);
&nagios_return("WARNING",  "Replication problem: Seconds_Behind_Master=".$result->{'Seconds_Behind_Master'}) if ($result->{'Seconds_Behind_Master'} > 60);
$sth->finish();
